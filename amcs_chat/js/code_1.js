var ClassRoom = {
    init: function(service, options) {
        ClassRoom.Core.init(service, options);
    }
};
...
ClassRoom.Core = {
    ...
    room: null,
    init: function(service, options) {
        ClassRoom.Core.service = service;
        ClassRoom.Core.room = options.room;
        ClassRoom.Core.addNamespaces();
        ClassRoom.Core.connection = new Strophe.Connection(service);
    },
    connect: function(jidOrHost, password, nick) {
        connection.reset();
    ...
    addNamespaces: function() {
        addNamespace('PRIVATE', 'jabber:iq:private');
        addNamespace('BOOKMARKS', 'storage:bookmarks');
        addNamespace('PRIVACY', 'jabber:iq:privacy');
        addNamespace('DELAY', 'jabber:x:delay');
        //addNamespace('NS_BOSH', 'http://localhost/http-bind');
        addNamespace('NS_MUC', 'http://jabber.org/protocol/muc');
        addNamespace('NS_OpenFireDomain', 'jabber');
        //addNamespace('NS_FIXED_ROOM', 'raum1@conference.jabber');
        addNamespace('NS_INSTANT_FEEDBACK', 'acms:ns:instantFeedback');
        addNamespace('NS_SLIDE', 'acms:ns:slide');
    },
    registerEventHandlers: function() {
        addHandler(self.Event.Jabber.Version, Strophe.NS.VERSION, 'iq');
        addHandler(self.Event.Jabber.Presence, null, 'presence');
        addHandler(self.Event.Jabber.Message, null, 'message');
        addHandler(self.Event.Jabber.Bookmarks, Strophe.NS.PRIVATE, 'iq');
        addHandler(self.Event.Jabber.Room.Disco, Strophe.NS.DISCO_INFO, 'iq');
        addHandler(self.Event.Jabber.PrivacyList, Strophe.NS.PRIVACY, 'iq', 'result');
        addHandler(self.Event.Jabber.PrivacyListError, Strophe.NS.PRIVACY, 'iq', 'error');
    },
    addHandler: function(handler, ns, name, type, id, from, options) {
        return connection.addHandler(handler, ns, name, type, id, from, options);
    }
